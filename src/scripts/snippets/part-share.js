...
function facebookShare() {
  const { url } = getDataset(event.target)
...
function twitterShare() {
  const { text, url } = getDataset(event.target)
...
function pinterestShare() {
  const { text, url, image } = getDataset(event.target)
...
function emailShare() {
  const { subject, url } = getDataset(event.target)
  const body = `\n\n--\n\n${url}`
